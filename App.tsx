...
const LeaderView: React.FC<{ user: User; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, machines, records, checklistItems }) => {
  ...
  // Nuevo estado para diferenciar si es mantenimiento o cierre de falla
  ...
  const requestMantoSignature = () => {
      ...
      if (!selectedMachine || !allChecked) return alert("Complete todos los items del checklist.");
      setPinAction('MANTO');
  ...
  // --- PROCESO DE GUARDADO CON PIN ---
  const handlePinConfirm = async (pin: string) => {
    // Validamos PIN contra el usuario actual
    if (pin !== user.pin) return alert("ERROR: PIN inválido.");
    
    try {
        // CASO A: MANTENIMIENTO PREVENTIVO (El de siempre)
        ...
        // CASO B: CIERRE DE FALLA (LA MEJORA DE TRAZABILIDAD)
        else if (pinAction === 'ISSUE' && closingIssue) {
            // 1. Apagamos la alerta roja del registro original (para que deje de salir como pendiente)
            // NO tocamos el texto original para respetar el reporte del operario.
            await updateDoc(doc(db, "records", closingIssue.id), { isIssue: false });

            // 2. Creamos un NUEVO registro de "Solución" en la auditoría
            // Este registro tendrá TU usuario (el Líder) y la fecha de HOY.
            await addDoc(collection(db, "records"), { 
                machineId: closingIssue.machineId, 
                userId: user.id, 
                date: new Date().toISOString(), 
                type: MaintenanceType.HEAVY, // Cuenta como trabajo técnico
                isIssue: false, // Es una solución, no un problema
                downtime: 0,
                // En observaciones ponemos claro que es una solución y referenciamos la falla
  ...
  if (selectedMachine) {
    return (
      <Card className="max-w-2xl mx-auto border-amber-600 shadow-amber-100/50 relative mb-20">
        {/* Usamos el mismo modal de PIN para ambas acciones */}
        ...
        {/* Botón apunta a requestMantoSignature */}
...
const ManagerView: React.FC<{ users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[]; onInitChecklist: () => void }> = ({ users, machines, records, checklistItems, onInitChecklist }) => {
  ...
  // FILTRO INTELIGENTE: Solo mostramos máquinas ACTIVAS en el selector de KPI y en los cálculos
