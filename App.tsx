...
const LeaderView: React.FC<{ user: User; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, machines, records, checklistItems }) => {
  ...
  // ESTADOS PARA LA GESTIÓN DE URGENCIAS
  const [closingIssue, setClosingIssue] = useState<MaintenanceRecord | null>(null);
  const [closingComment, setClosingComment] = useState('');
  
  // ESTADO PARA SABER QUÉ ESTAMOS FIRMANDO (MANTENIMIENTO O CIERRE DE FALLA)
  ...
  // 1. SOLICITAR FIRMA PARA MANTENIMIENTO PREVENTIVO
  ...
  // 2. SOLICITAR FIRMA PARA CERRAR FALLA (NUEVO)
  const requestIssueSignature = () => {
      if(!closingComment) return alert("Debe ingresar el detalle de la solución.");
      setPinAction('ISSUE');
      setShowPinModal(true);
  };

  // 3. PROCESO DE GUARDADO (CON LA MEJORA DE AUDITORÍA)
  const handlePinConfirm = async (pin: string) => {
    if (pin !== user.pin) return alert("ERROR: PIN inválido.");

    try {
        // OPCIÓN A: Guardar Mantenimiento Normal
        ...
        // OPCIÓN B: Cerrar Falla (ACÁ ESTÁ LA MEJORA)
        else if (pinAction === 'ISSUE' && closingIssue) {
            // 1. Quitamos la alerta roja del registro original (pero NO le cambiamos el texto, para que quede el reporte original)
            await updateDoc(doc(db, "records", closingIssue.id), { isIssue: false });

            // 2. CREAMOS UN REGISTRO NUEVO (Esto es lo que pediste)
            // Esto aparecerá como un renglón nuevo en la auditoría con tu nombre y lo que hiciste.
            await addDoc(collection(db, "records"), { 
                machineId: closingIssue.machineId, 
                userId: user.id, // Queda registrado TU usuario (Líder)
                date: new Date().toISOString(), // Fecha actual de la solución
                observations: `✅ SOLUCIÓN LÍDER: ${closingComment} (Ref. Falla: "${closingIssue.observations}")`, // Texto claro
                type: MaintenanceType.HEAVY, // Cuenta como trabajo pesado/técnico
                isIssue: false, // Es una solución, no un problema
                downtime: 0 
            });

            setClosingIssue(null); setClosingComment('');
            alert("Solución registrada y firmada correctamente.");
        }
  ...
  if (selectedMachine) {
    return (
      <Card className="max-w-2xl mx-auto border-amber-600 shadow-amber-100/50 relative mb-20">
        {/* El Modal de PIN se reutiliza aquí */}
        <PinModal isOpen={showPinModal} onClose={() => setShowPinModal(false)} onConfirm={handlePinConfirm} title="Firma de Responsable Técnico" />
        ...
        {/* INPUT DE TIEMPO SIN CERO MOLESTO */}
        <div className="mb-8">
          <label className="text-[10px] font-black uppercase text-slate-400 ml-2">Tiempo Parada (Min)</label>
          <input 
            type="number" 
  ...
  return (
    <div className="space-y-16 relative">
      ...
      {/* 1. EL MODAL DE CIERRE AHORA PIDE CONFIRMACIÓN Y MUESTRA EL CAMPO DE SOLUCION */}
      {closingIssue && (
      ...
      {/* 2. MODAL DE PIN GLOBAL PARA ESTA VISTA */}
...
// --- MANAGER VIEW (CON AUDITORIA BONITA) ---
...
const ManagerView: React.FC<{ users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[]; onInitChecklist: () => void }> = ({ users, machines, records, checklistItems, onInitChecklist }) => {
  ...
  // --- PARSER VISUAL DE AUDITORÍA (AGREGADO) ---
  const AuditParser = ({ text }: { text: string }) => {
    if (!text) return null;
    
    // Regex para detectar la solución del líder
    const solucionMatch = text.match(/✅ SOLUCIÓN LÍDER: (.*?) \(Ref/); 

    // Regex para el resto de los campos
    ...
    let cleanObs = text
      ...
      .replace(/\[HORAS MOTOR: .*?\]/, '')
      .replace(/\[PARADA: .*?\]/, '')
      .replace(/✅ SOLUCIÓN LÍDER: .*? \(Ref.*?\)/, '') 
      .trim();

    // Si es un registro de SOLUCIÓN, lo mostramos destacado
    if (text.includes("✅ SOLUCIÓN LÍDER:")) {
        ...
        return (
            <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                ...
                <p className="text-[9px] text-emerald-400 mt-2">Este registro cierra una incidencia anterior.</p>
            </div>
        );
    }
    ...
    return (
      <div className="space-y-3">
        {paradaMatch && (
           <span className="inline-block bg-slate-100 text-slate-600 font-black text-[10px] px-2 py-1 rounded mr-2">
             ⏱ Parada: {paradaMatch[1]} min
           </span>
        )}
        
        {checklistMatch && (
          <div className="flex flex-wrap gap-1 mb-2">
            {checklistMatch[1].split(', ').map((item, idx) => {
              ...
              return (
                <span key={idx} className={`text-[9px] px-2 py-1 rounded font-bold border border-slate-100 ${color}`}>
                  {key}: {val}
                </span>
              );
            })}
        ...
        {(docsMatch || horasMatch) && (
            <div className="bg-blue-50 p-2 rounded-lg border border-blue-100 mb-2">
                {docsMatch && <p className="text-xs font-bold text-blue-800">🚚 Datos: {docsMatch[1]}</p>}
                {horasMatch && <p className="text-xs font-bold text-amber-800">🏗 Horómetro: {horasMatch[1]}</p>}
            </div>
        )}

        {cleanObs && (
            <p className="text-sm text-slate-700 font-medium italic border-l-4 border-slate-200 pl-3">
                "{cleanObs}"
            </p>
        )}
      </div>
  ...
  return (
    <div className="space-y-12">
      ...
      {/* MODAL DETALLE AUDITORÍA (MEJORADO) */}
      {viewRecord && (
        <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setViewRecord(null)}>
            <Card className="w-full max-w-lg" onClick={(e: any) => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-6">
                    <div>
                        <h3 className="text-xl font-black uppercase text-slate-800">{machines.find(m => m.id === viewRecord.machineId)?.name}</h3>
                        <p className="text-xs text-slate-400 font-bold uppercase">{format(safeDate(viewRecord.date), 'dd/MM/yyyy HH:mm')}</p>
                    </div>
                    <button onClick={() => setViewRecord(null)}><X className="w-6 h-6 text-slate-400 hover:text-red-500" /></button>
                </div>
                <div className="space-y-4">
                    ...
                    <div className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm">
                        <p className="text-[10px] font-black uppercase text-slate-400 mb-3 border-b border-slate-100 pb-2">Detalle Técnico</p>
                        <AuditParser text={viewRecord.observations || ''} />
                    </div>
      ...
      <div className="flex flex-col xl:flex-row justify-between items-center gap-8 no-print">
          <div>
            ...
            <p className="text-slate-400 font-bold uppercase text-[10px] tracking-[0.3em] mt-3">
                Gerencia Técnica • Dashboard Intelligence
            </p>
          </div>
          <div className="flex bg-white p-2 rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-x-auto max-w-full">
              {[{ id: 'STATS', label: 'KPIs', icon: BarChart3 }, { id: 'HISTORY', label: 'Auditoría', icon: History }, { id: 'MACHINES', label: 'Activos', icon: HardDrive }, { id: 'USERS', label: 'Personal', icon: Users }, { id: 'CONFIG', label: 'Config', icon: ListChecks }].map(tab => (
      ...
      {activePanel === 'STATS' && (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        ...
        <div className="bg-slate-800 p-4 rounded-2xl flex justify-between items-center">
            ...
            <select className="bg-slate-700 text-white font-bold p-2 rounded-xl outline-none border border-slate-600" value={selectedMachineId} onChange={e => setSelectedMachineId(e.target.value)}>
                <option value="ALL">Vista Global (Solo Activas)</option>
                {activeMachinesList.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
            </select>
        </div>
        ...
        <div className="w-full"><MiniCalendar machines={activeMachinesList} records={records} users={users} mode="MANAGER" /></div>
                    ...
                    {users.filter(u => u.role === Role.OPERATOR || u.role === Role.LEADER).map(u => { 
                        ...
                        return (
                            <div key={u.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-orange-300 transition-colors">
                                <div><span className="font-black text-slate-800 uppercase text-xs tracking-tight block">{u.name}</span><span className={`text-[9px] font-bold uppercase ${u.role === Role.LEADER ? 'text-amber-600' : 'text-slate-400'}`}>{u.role === Role.LEADER ? 'Líder' : 'Operario'}</span></div>
                                <div className="flex items-center gap-3"><div className="flex flex-col items-end gap-1"><div className="flex items-center gap-1"><div className={`w-2 h-2 rounded-full ${volumeColor}`}></div><span className="text-[10px] font-black uppercase">Vol: {count}</span></div><span className={`text-[9px] font-black px-2 py-0.5 rounded border uppercase ${effColor}`}>Efec: {efficiencyRate}%</span></div><button onClick={() => openWAModal(u)}><MessageSquare className="w-5 h-5 text-emerald-500 cursor-pointer hover:scale-110 transition-transform" /></button></div>
                            </div>
                        ); 
                    })}
                </div>
