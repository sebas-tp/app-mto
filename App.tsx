...
type AssetType = 'MAQUINA' | 'VEHICULO' | 'MONTACARGA';
...
const MiniCalendar: React.FC<{ machines: ExtendedMachine[], records: MaintenanceRecord[], users?: User[], user?: User, mode: 'MANAGER' | 'OPERATOR' }> = ({ machines, records, users = [], user, mode }) => {
  ...
  const getDayStatus = (date: Date) => {
    ...
    const activeMachines = machines.filter(m => m.status !== 'STOPPED');

    const isPending = activeMachines.some(m => {
...
const OperatorView: React.FC<{ user: User; users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, users, machines, records, checklistItems }) => {
  ...
  const myChecklistItems = checklistItems.filter(i => { if (i.roleTarget !== Role.OPERATOR) return false; const currentMachineType = selectedMachine?.assetType || 'MAQUINA'; const itemTarget = i.targetType || 'ALL'; return itemTarget === 'ALL' || itemTarget === currentMachineType; });
  ...
  if (selectedMachine) {
    ...
    return (
      <Card className="max-w-2xl mx-auto border-orange-200 shadow-orange-100 relative mb-20">
            ...
            {myChecklistItems.map((item) => (
                <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border-2 border-slate-100 bg-white gap-4 sm:gap-0">
                    ...
                    <div className="flex gap-2 flex-wrap sm:flex-nowrap justify-end">
                        ...
                        {(isVehicle || isForklift) ? (<><button onClick={() => handleCheck(item.id, 'DEFICIENTE')} className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${checklistStatus[item.id] === 'DEFICIENTE' ? 'bg-amber-500 text-white shadow-amber-200 shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-amber-100 hover:text-amber-600'}`}>DEFICIENTE</button><button onClick={() => handleCheck(item.id, 'BIEN')} className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${checklistStatus[item.id] === 'BIEN' ? 'bg-emerald-500 text-white shadow-emerald-200 shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-emerald-100 hover:text-emerald-600'}`}>BIEN</button></>) : (<button onClick={() => handleCheck(item.id, 'OK')} className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${checklistStatus[item.id] === 'OK' ? 'bg-emerald-500 text-white shadow-emerald-200 shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-emerald-100 hover:text-emerald-600'}`}>HECHO</button>)}
                    </div>
                </div>
            ))}
        </div>
        ...
        <div className="mb-8"><label className="text-[10px] font-black uppercase text-slate-400 ml-2">Tiempo Parada (Min)</label><input type="number" className="w-full p-4 border-2 border-slate-100 rounded-[2rem] outline-none focus:border-orange-500 font-bold text-xl placeholder:text-slate-300" placeholder="0" value={downtime === 0 ? '' : downtime} onChange={e => setDowntime(parseInt(e.target.value) || 0)} /></div>
  ...
  return (
    <div className="space-y-12">
      ...
      {myMachines.length > 0 && (<div className="space-y-6"><h3 className="text-xl font-black text-slate-800 uppercase flex items-center gap-3"><UserCog className="text-orange-600" /> Mis Responsabilidades</h3><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">{myMachines.map(m => { const isDue = isPast(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)); return (<Card key={m.id} className={`${isDue ? 'border-red-500 shadow-red-100' : 'border-emerald-500 shadow-emerald-100'} cursor-pointer hover:scale-[1.02] transition-transform`} onClick={() => { setSelectedMachine(m); setChecklistStatus({}); }}><div className="flex justify-between mb-4"><div className={`p-3 rounded-2xl ${isDue ? 'bg-red-50 text-red-600' : 'bg-emerald-50 text-emerald-600'}`}>{m.assetType === 'VEHICULO' ? <Truck className="w-6 h-6"/> : m.assetType === 'MONTACARGA' ? <Container className="w-6 h-6"/> : <Wrench className="w-6 h-6" />}</div>{isDue && <span className="text-[9px] font-black bg-red-600 text-white px-3 py-1 rounded-full animate-pulse uppercase tracking-widest">Atención Requerida</span>}</div><h3 className="text-2xl font-black text-slate-800 uppercase mb-4 leading-tight">{m.name}</h3><div className="space-y-2"><p className="text-[10px] font-black text-slate-400 uppercase">Frecuencia: {m.operatorInterval} días</p></div></Card>); })}</div></div>)}
      {alertMachines.length > 0 && (<div className="space-y-6 pt-12 border-t border-slate-200"><h3 className="text-xl font-black text-red-600 uppercase flex items-center gap-3 animate-pulse"><AlertTriangle /> Mantenimiento Vencido (General)</h3><div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6">{alertMachines.map(m => { const respUser = users.find(u => u.id === m.operatorId); return (<div key={m.id} onClick={() => { setSelectedMachine(m); setChecklistStatus({}); }} className="bg-red-50 p-6 rounded-3xl border border-red-200 cursor-pointer hover:shadow-xl transition-all flex flex-col justify-between items-start group"><div className="flex justify-between w-full mb-4"><div className="bg-white p-2 rounded-xl text-red-500"><Wrench className="w-5 h-5" /></div><span className="text-[9px] bg-red-200 text-red-800 px-2 py-1 rounded font-bold uppercase">Vencido</span></div><p className="font-black text-slate-800 uppercase tracking-tighter leading-none mb-2">{m.name}</p><p className="text-[9px] font-black text-red-400 uppercase tracking-widest">Resp: {respUser ? respUser.name : 'Sin Asignar'}</p></div>); })}</div></div>)}
      <div className="space-y-6 pt-12 border-t border-slate-200"><div className="flex flex-col md:flex-row justify-between items-end gap-4"><h3 className="text-xl font-black text-slate-800 uppercase flex items-center gap-3"><ScanLine className="text-slate-400" /> Operar Otro Equipo</h3><div className="flex items-center gap-2 p-3 bg-white rounded-xl border border-slate-200 w-full md:w-auto"><Search className="w-4 h-4 text-slate-400" /><input className="outline-none text-sm font-bold text-slate-700 bg-transparent" placeholder="Buscar equipo..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} /></div></div>{searchTerm && (<div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-6 animate-in fade-in">{otherMachines.map(m => { return (<div key={m.id} onClick={() => { setSelectedMachine(m); setChecklistStatus({}); }} className="bg-white p-6 rounded-3xl border border-slate-100 cursor-pointer hover:shadow-xl transition-all flex flex-col justify-between items-start group"><div className="flex justify-between w-full mb-4"><div className="bg-slate-100 p-2 rounded-xl text-slate-500 group-hover:text-orange-600 transition-colors">{m.assetType === 'VEHICULO' ? <Truck className="w-5 h-5"/> : m.assetType === 'MONTACARGA' ? <Container className="w-5 h-5"/> : <HardDrive className="w-5 h-5" />}</div><span className="text-[9px] bg-emerald-100 text-emerald-600 px-2 py-1 rounded font-bold uppercase">Al Día</span></div><p className="font-black text-slate-800 uppercase tracking-tighter leading-none mb-2">{m.name}</p><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Ciclo: {m.operatorInterval}d</p></div>); })}</div>)}</div>
    </div>
  );
};

// --- LEADER VIEW (MEJORADA: PIN + NUEVO REGISTRO EN AUDITORIA) ---
...
const LeaderView: React.FC<{ user: User; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, machines, records, checklistItems }) => {
  ...
  const [pinAction, setPinAction] = useState<'MANTO' | 'ISSUE'>('MANTO');
  ...
  const myChecklistItems = checklistItems.filter(i => { if (i.roleTarget !== Role.LEADER) return false; const currentMachineType = selectedMachine?.assetType || 'MAQUINA'; const itemTarget = i.targetType || 'ALL'; return itemTarget === 'ALL' || itemTarget === currentMachineType; });

  const handleCheck = (itemId: string, status: string) => { setChecklistStatus(prev => { if (prev[itemId] === status) { const n = { ...prev }; delete n[itemId]; return n; } return { ...prev, [itemId]: status }; }); };
  const requestMantoSignature = () => { const allChecked = myChecklistItems.every(i => checklistStatus[i.id]); if (!selectedMachine || !allChecked) return alert("Complete todos los items."); setPinAction('MANTO'); setShowPinModal(true); };
  const requestIssueSignature = () => { if(!closingComment) return alert("Debe ingresar el detalle de la solución."); setPinAction('ISSUE'); setShowPinModal(true); };
  
  const handlePinConfirm = async (pin: string) => {
    if (pin !== user.pin) return alert("ERROR: PIN inválido.");
    try {
        if (pinAction === 'MANTO' && selectedMachine) {
            const checklistText = Object.entries(checklistStatus).map(([id, s]) => `${checklistItems.find(i => i.id === id)?.label}: ${s}`).join(', ');
            ...
            setSelectedMachine(null); setChecklistStatus({}); setMantoObs(''); setDowntime(0); alert("Mantenimiento Certificado.");
        } else if (pinAction === 'ISSUE' && closingIssue) {
            await updateDoc(doc(db, "records", closingIssue.id), { isIssue: false });
            await addDoc(collection(db, "records"), { machineId: closingIssue.machineId, userId: user.id, date: new Date().toISOString(), type: MaintenanceType.HEAVY, isIssue: false, downtime: 0, observations: `✅ SOLUCIÓN LÍDER: ${closingComment} (Ref. Falla: "${closingIssue.observations}")` });
            setClosingIssue(null); setClosingComment(''); alert("Solución registrada y firmada.");
        }
  ...
  if (selectedMachine) {
    return (
      <Card className="max-w-2xl mx-auto border-amber-600 shadow-amber-100/50 relative mb-20">
        <PinModal isOpen={showPinModal} onClose={() => setShowPinModal(false)} onConfirm={handlePinConfirm} title={pinAction === 'MANTO' ? "Firma Mto. Preventivo" : "Firma Solución"} />
        ...
        <div className="space-y-4 mb-8">{myChecklistItems.map((item) => (<div key={item.id} className="flex items-center justify-between p-4 rounded-2xl border-2 border-slate-100 bg-white"><span className="text-sm font-bold text-slate-700 w-1/2">{item.label}</span><div className="flex gap-2"><button onClick={() => handleCheck(item.id, 'NA')} className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${checklistStatus[item.id] === 'NA' ? 'bg-slate-600 text-white' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>N/A</button><button onClick={() => handleCheck(item.id, 'OK')} className={`px-3 py-2 rounded-xl text-[10px] font-black transition-all ${checklistStatus[item.id] === 'OK' ? 'bg-amber-600 text-white shadow-amber-200 shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-amber-100 hover:text-amber-600'}`}>HECHO</button></div></div>))}</div>
        <div className="mb-8"><label className="text-[10px] font-black uppercase text-slate-400 ml-2">Tiempo Parada (Min)</label><input type="number" className="w-full p-4 border-2 border-slate-100 rounded-[2rem] outline-none focus:border-amber-500 font-bold text-xl placeholder:text-slate-300" placeholder="0" value={downtime === 0 ? '' : downtime} onChange={e => setDowntime(parseInt(e.target.value) || 0)} /></div>
        ...
        <IndustrialButton variant="secondary" fullWidth onClick={requestMantoSignature}>Firmar Mantenimiento Experto</IndustrialButton>
      </Card>
    );
  }
...
const ManagerView: React.FC<{ users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[]; onInitChecklist: () => void }> = ({ users, machines, records, checklistItems, onInitChecklist }) => {
  ...
  const [newItemTarget, setNewItemTarget] = useState<AssetType | 'ALL'>('ALL');
  const [showWAModal, setShowWAModal] = useState(false);
  const [waTargetUser, setWaTargetUser] = useState<User | null>(null);

  const activeMachinesList = useMemo(() => machines.filter(m => m.status !== 'STOPPED'), [machines]);
  const filteredMachines = useMemo(() => selectedMachineId === 'ALL' ? activeMachinesList : activeMachinesList.filter(m => m.id === selectedMachineId), [activeMachinesList, selectedMachineId]);
  ...
  const visibleMachinesInList = useMemo(() => { return machines.filter(m => (m.name || '').toLowerCase().includes(machineSearch.toLowerCase())); }, [machines, machineSearch]);
  ...
  const kpiData = useMemo(() => {
    ...
    const compliantMachines = filteredMachines.filter(m => { const opDue = isPast(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)) && !isToday(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)); const ldDue = isPast(addDays(safeDate(m.lastLeaderDate), m.leaderInterval || 30)) && !isToday(addDays(safeDate(m.lastLeaderDate), m.leaderInterval || 30)); return !opDue && !ldDue; }).length;
  ...
  const stats = useMemo(() => { const total = filteredMachines.length; const due = filteredMachines.filter(m => { const opDue = isPast(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)) && !isToday(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)); const leadDue = isPast(addDays(safeDate(m.lastLeaderDate), m.leaderInterval || 30)) && !isToday(addDays(safeDate(m.lastLeaderDate), m.leaderInterval || 30)); return opDue || leadDue; }).length; return [{ name: 'Operativo', value: total - due, color: '#10b981' }, { name: 'Vencido', value: due, color: '#ef4444' }]; }, [filteredMachines]);
  const trendData = useMemo(() => { const grouped = filteredRecordsForStats.reduce((acc: any, r) => { const date = r.date.slice(0, 10); if (!acc[date]) acc[date] = { date, downtime: 0, preventive: 0, total: 0 }; acc[date].downtime += (r.downtime || 0); if (!r.isIssue) acc[date].preventive += 1; acc[date].total += 1; return acc; }, {}); return Object.values(grouped).map((day: any) => ({ ...day, effectiveness: day.total > 0 ? Math.round((day.preventive / day.total) * 100) : 0 })).sort((a: any, b: any) => new Date(a.date).getTime() - new Date(b.date).getTime()); }, [filteredRecordsForStats]);
  const planVsRealData = useMemo(() => { const preventive = filteredRecordsForStats.filter(r => !r.isIssue).length; const corrective = filteredRecordsForStats.filter(r => r.isIssue).length; return [{ name: 'Plan (Preventivo)', value: preventive }, { name: 'Incidencias (Correctivo)', value: corrective }]; }, [filteredRecordsForStats]);
  ...
  const exportToCSV = () => { const headers = "Fecha,Hora,Maquina,Usuario,Tipo,Observaciones\n"; const rows = filteredRecords.map(r => { const u = users.find(u => u.id === r.userId)?.name || "Desconocido"; const m = machines.find(m => m.id === r.machineId)?.name || "Eliminada"; const t = r.isIssue ? "FALLA" : "Mantenimiento"; const obs = (r.observations || "").replace(/,/g, " "); return `${format(parseISO(r.date), 'dd/MM/yyyy')},${format(parseISO(r.date), 'HH:mm')},${m},${u},${t},${obs}`; }).join("\n"); const blob = new Blob([headers + rows], { type: 'text/csv;charset=utf-8;' }); const link = document.createElement("a"); const url = URL.createObjectURL(blob); link.setAttribute("href", url); link.setAttribute("download", `auditoria_mantenimiento_${format(new Date(), 'yyyy-MM-dd')}.csv`); document.body.appendChild(link); link.click(); document.body.removeChild(link); };
  ...
  const handleDeleteRecord = async (recordId: string, e: React.MouseEvent) => { e.stopPropagation(); if (!window.confirm("¿Seguro que desea eliminar este registro de auditoría? Esta acción no se puede deshacer.")) return; try { await deleteDoc(doc(db, "records", recordId)); } catch (e) { console.error("Error eliminando:", e); alert("Error al eliminar."); } };

  // --- PARSER VISUAL DE AUDITORÍA (NUEVO) ---
  const AuditParser = ({ text }: { text: string }) => {
    if (!text) return null;
    const checklistMatch = text.match(/\[CHECKLIST: (.*?)\]/);
    const docsMatch = text.match(/\[DOCS\/KM: (.*?)\]/);
    const horasMatch = text.match(/\[HORAS MOTOR: (.*?)\]/);
    const paradaMatch = text.match(/\[PARADA: (.*?) min\]/);
    
    // Si detectamos solución, mostramos tarjeta especial
    if (text.includes("✅ SOLUCIÓN LÍDER:")) {
        const solutionText = text.split('(Ref. Falla:')[0].replace('✅ SOLUCIÓN LÍDER:', '').trim();
        return (
            <div className="bg-emerald-50 p-3 rounded-xl border border-emerald-100">
                <p className="text-[10px] font-black text-emerald-600 uppercase mb-1">🔧 Acción Correctiva:</p>
                <p className="text-sm font-bold text-emerald-800 italic">"{solutionText}"</p>
                <p className="text-[9px] text-emerald-400 mt-2">Cierre de incidencia.</p>
            </div>
        );
    }

    let cleanObs = text.replace(/\[CHECKLIST: .*?\]/, '').replace(/\[DOCS\/KM: .*?\]/, '').replace(/\[HORAS MOTOR: .*?\]/, '').replace(/\[PARADA: .*?\]/, '').trim();

    return (
      <div className="space-y-3">
        {paradaMatch && (<span className="inline-block bg-slate-100 text-slate-600 font-black text-[10px] px-2 py-1 rounded mr-2">⏱ Parada: {paradaMatch[1]} min</span>)}
        {checklistMatch && (<div className="flex flex-wrap gap-1 mb-2">{checklistMatch[1].split(', ').map((item, idx) => { const [key, val] = item.split(': '); let color = 'bg-slate-100 text-slate-500'; if(['OK','BIEN','HECHO'].includes(val)) color = 'bg-emerald-100 text-emerald-700'; if(['DEFICIENTE','MAL'].includes(val)) color = 'bg-red-100 text-red-700'; return (<span key={idx} className={`text-[9px] px-2 py-1 rounded font-bold border border-slate-100 ${color}`}>{key}: {val}</span>); })}</div>)}
        {(docsMatch || horasMatch) && (<div className="bg-blue-50 p-2 rounded-lg border border-blue-100 mb-2">{docsMatch && <p className="text-xs font-bold text-blue-800">🚚 Datos: {docsMatch[1]}</p>}{horasMatch && <p className="text-xs font-bold text-amber-800">🏗 Horómetro: {horasMatch[1]}</p>}</div>)}
        {cleanObs && (<p className="text-sm text-slate-700 font-medium italic border-l-4 border-slate-200 pl-3">"{cleanObs}"</p>)}
      </div>
    );
  };
  ...
  return (
    <div className="space-y-12">
      ...
      {viewRecord && (<div className="fixed inset-0 bg-slate-900/80 backdrop-blur-sm z-[200] flex items-center justify-center p-4 animate-in fade-in duration-200" onClick={() => setViewRecord(null)}><Card className="w-full max-w-lg" onClick={(e: any) => e.stopPropagation()}><div className="flex justify-between items-start mb-6"><div><h3 className="text-xl font-black uppercase text-slate-800">{machines.find(m => m.id === viewRecord.machineId)?.name}</h3><p className="text-xs text-slate-400 font-bold uppercase">{format(safeDate(viewRecord.date), 'dd/MM/yyyy HH:mm')}</p></div><button onClick={() => setViewRecord(null)}><X className="w-6 h-6 text-slate-400 hover:text-red-500" /></button></div><div className="space-y-4"><div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl"><span className="text-sm font-bold text-slate-600">Responsable</span><span className="text-sm font-black text-slate-900 uppercase">{(users.find(u => u.id === viewRecord.userId)?.name) || 'Desconocido'}</span></div><div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl"><span className="text-sm font-bold text-slate-600">Tipo Registro</span><span className={`text-[10px] font-black px-3 py-1 rounded-full uppercase ${viewRecord.isIssue ? 'bg-red-100 text-red-700' : 'bg-emerald-100 text-emerald-700'}`}>{viewRecord.isIssue ? 'CORRECTIVO' : 'PREVENTIVO'}</span></div><div className="p-4 bg-white border border-slate-100 rounded-2xl shadow-sm"><p className="text-[10px] font-black uppercase text-slate-400 mb-3 border-b border-slate-100 pb-2">Detalle Técnico</p><AuditParser text={viewRecord.observations || ''} /></div>{viewRecord.downtime > 0 && (<div className="flex items-center gap-3 p-4 bg-red-50 border border-red-100 rounded-2xl"><Timer className="w-5 h-5 text-red-500" /><span className="text-sm font-black text-red-700">Tiempo de Parada: {viewRecord.downtime} minutos</span></div>)}</div></Card></div>)}
      ...
      {activePanel === 'STATS' && (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500"><div className="bg-slate-800 p-4 rounded-2xl flex justify-between items-center"><h3 className="text-white font-bold uppercase text-sm flex items-center gap-2"><Filter className="w-4 h-4 text-orange-500"/> Filtro de Planta:</h3><select className="bg-slate-700 text-white font-bold p-2 rounded-xl outline-none border border-slate-600" value={selectedMachineId} onChange={e => setSelectedMachineId(e.target.value)}><option value="ALL">Vista Global (Solo Activas)</option>{activeMachinesList.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}</select></div><div className="grid grid-cols-2 lg:grid-cols-4 gap-4"><Card className="bg-slate-900 text-white border-none"><div className="flex items-center gap-3 mb-2"><HardDrive className="text-orange-500 w-5 h-5" /><span className="text-[10px] font-black uppercase tracking-widest opacity-70">Activos en Uso</span></div><p className="text-4xl font-black">{kpiData.totalAssets}</p></Card><Card className="bg-white border-emerald-100"><div className="flex items-center gap-3 mb-2"><Activity className="text-emerald-500 w-5 h-5" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Cumplimiento (Plan)</span></div><p className="text-4xl font-black text-emerald-600">{kpiData.complianceRate}%</p></Card><Card className="bg-white border-red-100"><div className="flex items-center gap-3 mb-2"><Timer className="text-red-500 w-5 h-5" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Tiempo Parada</span></div><p className="text-4xl font-black text-red-600">{kpiData.totalDowntime}<span className="text-sm text-slate-400 ml-1">min</span></p></Card><Card className="bg-white border-blue-100"><div className="flex items-center gap-3 mb-2"><Gauge className="text-blue-500 w-5 h-5" /><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Eficiencia (OEE)</span></div><p className="text-4xl font-black text-blue-600">{kpiData.efficiency}%</p></Card></div><div className="grid grid-cols-1 lg:grid-cols-3 gap-8"><Card className="flex flex-col items-center justify-center"><h3 className="text-lg font-black uppercase text-slate-700 w-full text-center mb-4">Salud del Parque</h3><div className="h-[200px] w-full"><ResponsiveContainer><PieChart><Pie data={stats} cx="50%" cy="50%" innerRadius={60} outerRadius={80} paddingAngle={5} dataKey="value">{stats.map((e, i) => <Cell key={i} fill={e.color} strokeWidth={0} />)}</Pie><Tooltip contentStyle={{borderRadius: '16px', border: 'none', boxShadow: '0 10px 15px -3px rgb(0 0 0 / 0.1)'}} /><Legend verticalAlign="bottom" /></PieChart></ResponsiveContainer></div></Card><Card className="lg:col-span-2"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-black uppercase text-slate-800 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-orange-600"/> Evolución Histórica</h3><div className="flex bg-slate-100 p-1 rounded-xl"><button onClick={() => setChartMetric('EFFECTIVENESS')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${chartMetric === 'EFFECTIVENESS' ? 'bg-white shadow text-emerald-700' : 'text-slate-400'}`}>% Efectividad</button><button onClick={() => setChartMetric('DOWNTIME')} className={`px-4 py-2 rounded-lg text-xs font-bold transition-all ${chartMetric === 'DOWNTIME' ? 'bg-white shadow text-red-700' : 'text-slate-400'}`}>T. Parada</button></div></div><div className="h-64 w-full"><ResponsiveContainer width="100%" height="100%"><AreaChart data={trendData}><defs><linearGradient id="colorGreen" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#10b981" stopOpacity={0.8}/><stop offset="95%" stopColor="#10b981" stopOpacity={0}/></linearGradient><linearGradient id="colorRed" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#ef4444" stopOpacity={0.8}/><stop offset="95%" stopColor="#ef4444" stopOpacity={0}/></linearGradient></defs><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="date" axisLine={false} tickLine={false} tick={{fontSize: 10, fill: '#94a3b8'}} /><YAxis axisLine={false} tickLine={false} tick={{fontSize: 10, fill: '#94a3b8'}} /><Tooltip /><Area type="monotone" dataKey={chartMetric === 'DOWNTIME' ? 'downtime' : 'effectiveness'} stroke={chartMetric === 'DOWNTIME' ? '#ef4444' : '#10b981'} fillOpacity={1} fill={chartMetric === 'DOWNTIME' ? 'url(#colorRed)' : 'url(#colorGreen)'} /></AreaChart></ResponsiveContainer></div></Card></div><div className="w-full"><MiniCalendar machines={activeMachinesList} records={records} users={users} mode="MANAGER" /></div><div className="grid grid-cols-1 lg:grid-cols-2 gap-8"><Card><h3 className="text-xl font-black uppercase text-slate-700 border-b pb-6 mb-8 flex items-center gap-3"><BarChart3 className="text-orange-600" /> Plan vs. Incidencias (Volumen)</h3><div className="h-[300px] w-full"><ResponsiveContainer width="100%" height="100%"><BarChart data={planVsRealData}><CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#f1f5f9" /><XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} /><YAxis axisLine={false} tickLine={false} tick={{fontSize: 10, fontWeight: 'bold', fill: '#94a3b8'}} /><Tooltip cursor={{fill: '#f8fafc'}} contentStyle={{borderRadius: '12px', border: 'none', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)'}} /><Bar dataKey="value" fill="#f97316" radius={[10, 10, 0, 0]} barSize={60} /></BarChart></ResponsiveContainer></div></Card><Card><h3 className="text-xl font-black uppercase text-slate-700 border-b pb-6 mb-8 flex items-center gap-3"><Users className="text-orange-600" /> Productividad & Calidad</h3><div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">{users.filter(u => u.role === Role.OPERATOR || u.role === Role.LEADER).map(u => { const userRecords = records.filter(r => r.userId === u.id); const count = userRecords.length; const preventiveCount = userRecords.filter(r => !r.isIssue).length; const efficiencyRate = count > 0 ? Math.round((preventiveCount / count) * 100) : 100; const volumeColor = count > 10 ? 'bg-emerald-500' : count > 5 ? 'bg-yellow-500' : 'bg-red-500'; const effColor = efficiencyRate > 90 ? 'text-emerald-700 bg-emerald-50 border-emerald-200' : efficiencyRate > 70 ? 'text-yellow-700 bg-yellow-50 border-yellow-200' : 'text-red-700 bg-red-50 border-red-200'; return (<div key={u.id} className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:border-orange-300 transition-colors"><div><span className="font-black text-slate-800 uppercase text-xs tracking-tight block">{u.name}</span><span className={`text-[9px] font-bold uppercase ${u.role === Role.LEADER ? 'text-amber-600' : 'text-slate-400'}`}>{u.role === Role.LEADER ? 'Líder' : 'Operario'}</span></div><div className="flex items-center gap-3"><div className="flex flex-col items-end gap-1"><div className="flex items-center gap-1"><div className={`w-2 h-2 rounded-full ${volumeColor}`}></div><span className="text-[10px] font-black uppercase">Vol: {count}</span></div><span className={`text-[9px] font-black px-2 py-0.5 rounded border uppercase ${effColor}`}>Efec: {efficiencyRate}%</span></div><button onClick={() => openWAModal(u)}><MessageSquare className="w-5 h-5 text-emerald-500 cursor-pointer hover:scale-110 transition-transform" /></button></div></div>); })}</div></Card></div></div>)}
      ...
      {activePanel === 'MACHINES' && (<div className="grid grid-cols-1 gap-12"><Card><form onSubmit={handleMachineSubmit} className="space-y-6"><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter flex items-center gap-3"><Plus className="text-orange-600" /> {editingMachineId ? 'Editar' : 'Nuevo'} Activo</h3><input className="w-full p-5 rounded-2xl border-2" placeholder="Nombre Máquina" value={machineForm.name} onChange={e => setMachineForm({...machineForm, name: e.target.value})} /><div className="flex gap-2"><select className="w-full p-5 rounded-2xl border-2" value={machineForm.assetType} onChange={(e:any) => setMachineForm({...machineForm, assetType: e.target.value})}><option value="MAQUINA">MÁQUINA</option><option value="VEHICULO">VEHÍCULO</option><option value="MONTACARGA">MONTACARGA</option></select></div><IndustrialButton fullWidth type="submit">Guardar</IndustrialButton></form></Card><div className="space-y-4">{visibleMachinesInList.map((m:any) => (<Card key={m.id} className="flex justify-between items-center"><span className="font-black uppercase">{m.name}</span><div className="flex gap-2"><button onClick={() => handleEditMachine(m)}><Pencil className="w-4 h-4 text-blue-500"/></button><button onClick={() => handleDeleteMachine(m.id)}><Trash2 className="w-4 h-4 text-red-500"/></button></div></Card>))}</div></div>)}
      {activePanel === 'USERS' && (<div className="grid grid-cols-1 gap-12"><Card><form onSubmit={addUser} className="space-y-6"><h3 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">Nuevo Usuario</h3><input className="w-full p-5 rounded-2xl border-2" placeholder="Nombre" value={userForm.name} onChange={e => setUserForm({...userForm, name: e.target.value})} /><input className="w-full p-5 rounded-2xl border-2" placeholder="PIN" value={userForm.pin} onChange={e => setUserForm({...userForm, pin: e.target.value})} /><select className="w-full p-5 rounded-2xl border-2" value={userForm.role} onChange={(e:any) => setUserForm({...userForm, role: e.target.value})}><option value={Role.OPERATOR}>OPERARIO</option><option value={Role.LEADER}>LIDER</option><option value={Role.MANAGER}>GERENTE</option></select><IndustrialButton fullWidth type="submit">Crear</IndustrialButton></form></Card><div className="grid gap-4">{users.map((u:any) => (<Card key={u.id} className="flex justify-between"><span>{u.name} ({u.role})</span><button onClick={() => deleteUser(u.id)}><Trash2 className="text-red-500 w-4 h-4"/></button></Card>))}</div></div>)}
      {activePanel === 'CONFIG' && (<div className="grid grid-cols-1 gap-8"><Card><form onSubmit={addChecklistItem} className="space-y-6"><h3 className="text-2xl font-black">Nueva Tarea Checklist</h3><input className="w-full p-5 rounded-2xl border-2" placeholder="Tarea" value={newItemText} onChange={e => setNewItemText(e.target.value)} /><select className="w-full p-5 rounded-2xl border-2" value={newItemTarget} onChange={(e:any) => setNewItemTarget(e.target.value)}><option value="ALL">Todo</option><option value="MAQUINA">Máquina</option><option value="VEHICULO">Vehículo</option><option value="MONTACARGA">Montacarga</option></select><IndustrialButton fullWidth type="submit">Agregar</IndustrialButton></form></Card><div className="space-y-2">{checklistItems.map((i:any) => (<div key={i.id} className="flex justify-between p-4 bg-white border rounded-xl"><span>{i.label} ({i.targetType})</span><button onClick={() => deleteChecklistItem(i.id)}><Trash2 className="w-4 h-4 text-red-500"/></button></div>))}</div></div>)}
    </div>
  );
}
