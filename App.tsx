...
// --- VISTAS DEL SISTEMA (DEFINIDAS ANTES DE APP) ---
...
const OperatorView: React.FC<{ user: User; users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, users, machines, records, checklistItems }) => {
  ...
  const alertMachines = machines.filter(m => m.operatorId !== user.id && isPast(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)));
  const otherMachines = machines.filter(m => m.operatorId !== user.id && !isPast(addDays(safeDate(m.lastOperatorDate), m.operatorInterval || 15)) && (m.name || '').toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  // FILTRO INTELIGENTE DE CHECKLIST: Compatibilidad con datos viejos
  ...
  const finalizeManto = async (pin: string) => { 
    ...
    try { 
      ...
      // LOGICA ESPECIAL: Incluir documentación en observaciones
...
const ManagerView: React.FC<{ users: User[]; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[]; onInitChecklist: () => void }> = ({ users, machines, records, checklistItems, onInitChecklist }) => {
  ...
  const [machineForm, setMachineForm] = useState({ name: '', operatorInterval: 15, leaderInterval: 30, operatorId: '', leaderId: '', baseDate: new Date().toISOString().slice(0, 10), assetType: 'MAQUINA' });
