...
// Extendemos la interfaz Machine para incluir el tipo
...
interface ChecklistItem {
  ...
  label: string;
  roleTarget: Role; 
  targetType: AssetType | 'ALL'; // Nuevo filtro: ¿Aplica a qué?
}
  ...
  const getDayStatus = (date: Date) => {
    ...
    const isPending = machines.some(m => {
        ...
        if (mode === 'MANAGER') return opMatch || leadMatch;
            ...
            // Operario ve lo asignado y lo "huerfano"
...
const OperatorView: React.FC<{ user: User; machines: ExtendedMachine[]; records: MaintenanceRecord[]; checklistItems: ChecklistItem[] }> = ({ user, machines, records, checklistItems }) => {
  ...
  const commonMachines = machines.filter(m => !m.operatorId);
  ...
  return (
    <div className="space-y-12">
      ...
      {activePanel === 'STATS' && (<div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
        {/* ... (DASHBOARD COMPLETO DE LA VERSION ANTERIOR) ... */}
        {/* ... (REUTILIZA EL CÓDIGO DE KPI CARDS, GRAFICOS Y LISTA DE USUARIOS DE MI RESPUESTA ANTERIOR) ... */}
